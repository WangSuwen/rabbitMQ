...
//连接成功后创建通道
  ...
  /**
   * 通道创建成功后我们通过通道对象的assertQueue方法来监听hello队列，并设置durable持久化为false。
   * 这里消息将会被保存在内存中。该方法会返回一个promise对象。
   */
    var ok = ch.assertQueue('hello',{durable:false}).then(function(_qok){
      /**
       * 监听创建成功后，我们使用ch.consume创建一个消费者。指定消费hello队列和处理函数，在这里我们简单打印一句话。
       * 设置noAck为true表示不对消费结果做出回应。
